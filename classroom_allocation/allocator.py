...
def allocate():
    df = pd.read_csv("active_data/admit_data.csv", dtype=str)
    ...
    existing.drop_duplicates(
        subset=["reg_no", "name", "email"], inplace=True, ignore_index=True
    )
    e_list = existing["email"].to_list()
    existing = existing.filter(["email", "category", "classroom"]).join(
        df.set_index(["email", "category"]), on=["email", "category"]
    )
    ...
    df["email"] = (
        ...
        .str.lower()
        .map(
            lambda e: e if email_re.match(e) and e.endswith("@gmail.com") else None,
            na_action="ignore",
        )
    )
    df["email"] = df["email"][~df["email"].isin(e_list)]
    df = df.dropna(subset=["email", "zone", "category", "medium", "contact"]).sample(
        frac=1
    )
    ...
    df = pd.concat([existing] + dfs_junior + dfs_senior, ignore_index=True).filter(
        [
            "reg_no",
            "name",
            "email",
